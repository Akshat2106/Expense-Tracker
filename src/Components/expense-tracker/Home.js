...
const Home = () => {
  ...
  useEffect(() => {
    fetch(
      `https://expensetracker-5f883-default-rtdb.firebaseio.com/expenses/${email}.json`
    )
      .then((res) => res.json())
      .then((data) => {
        const items = [];
  ...
  const onSubmitHandler = async (e) => {
    ...
    let url = `https://expensetracker-5f883-default-rtdb.firebaseio.com/expenses/${email}.json`;
    let method = "POST";

    if (existingId) {
      url = `https://expensetracker-5f883-default-rtdb.firebaseio.com/expenses/${email}/${existingId}.json`;
      method = "PUT";
    }
  ...
  const deleteExpense = async (id) => {
    const res = await fetch(
      `https://expensetracker-5f883-default-rtdb.firebaseio.com/expenses/${email}/${id}.json`,
      {
        method: "DELETE",
      }
    );
        ...
        <div className="row">
          <div className="col-md-6 col-10 mx-auto">
            <div className="card bg-dark text-white">
              <h2 className="card-header bg-secondary">ADD YOUR EXPENSES HERE</h2>
